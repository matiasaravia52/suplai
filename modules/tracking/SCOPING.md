...
|----|---------------|-------------|----------------------|-----|
| `visit_tracking` | Registro de visitas | Check-in/out manual con captura de GPS en cada punto de cliente registrado | true | ✓ |
| `live_status` | Estado en tiempo real | Ver qué empleados están activos y en qué punto están ahora | true | ✓ |
| `visit_history` | Historial de visitas | Ver visitas pasadas por empleado, cliente o fecha, con exportación a Excel | true | ✓ |
| `route_tracing` | Trazabilidad de recorrido | Registro GPS continuo del recorrido completo (muestreo cada 30 segundos) | false | ✓ |
| `fraud_detection` | Validación de posición | Alerta cuando el GPS al check-in está a más de 150m de la ubicación del cliente | false | ✓ |
...
| `coverage_gaps` | Gaps de cobertura | Identificar clientes que no fueron visitados en un período | false | ✗ |

## Permisos

| Permiso | Feature | Quién lo tiene |
|---------|---------|---------------|
| `tracking:visit_tracking:create` | `visit_tracking` | `repartidor`, `pre_vendedor` |
| `tracking:visit_tracking:view` | `visit_tracking` | `coordinador` |
| `tracking:live_status:view` | `live_status` | `coordinador` |
| `tracking:visit_history:view` | `visit_history` | `coordinador` |
| `tracking:visit_history:export` | `visit_history` | `coordinador`, `tenant_admin` |
| `tracking:route_tracing:view` | `route_tracing` | `coordinador` |
| `tracking:fraud_detection:view` | `fraud_detection` | `coordinador`, `tenant_admin` |
...
|-------|------|-------------------|-------------------|
| Panel de campo | `/tracking` | `tracking:live_status:view` | `live_status` |
| Historial de visitas | `/tracking/historial` | `tracking:visit_history:view` | `visit_history` |
| Alertas de posición | `/tracking/alertas` | `tracking:fraud_detection:view` | `fraud_detection` |
...
- **Sin notificaciones en tiempo real al coordinador.** El estado se actualiza al entrar al panel. Se puede revisar en futuras versiones.
